...
`javascript:window.setInterval(function(){ console.log('attempting to merge...'); Array.from(document.getElementsByTagName("button")).filter(function(btn){ return btn.innerText in {'Update branch':1, 'Merge pull request':2, 'Confirm merge':3, 'Try again': 4, 'Delete branch': 5, 'Squash and merge': 6, 'Confirm squash and merge': 7} && !btn.disabled}).map(function(btn){btn.click()}) }, 500)`
